...
# Mainland China, Singapore, Thailand
dfObj = pd.DataFrame(columns=newColumns)  # Now, let's start populating dfObj:


for day in range(0, max(df['Last Update'])):  # for all days 0, 1, ..., 191
    ...
    for country in uniqueCountry:
        if (country in aa.iloc[:, 0].keys().tolist()):
            ...
            values.append(0)
    # Get all closing prices for that day
...
#
# model = LinearRegression(fit_intercept=False).fit(X, Y) # this is just a preliminary multilinear model with all 30 symbols
#
# Y = Y.flatten() # remove the list encasing number
# Y = list(Y)     # and make it comma separated, because we're about to calculate the pearson coefficient of correlation
#
...
#  stockRankings[stockID] = [pearson_coef, p_value, modelCoefficients[stockID], stockID, symbolsOriginal[stockID]]
...
#  ax[math.floor(stockID / 6), stockID % 6].scatter(prices130DaysForStock, Y, s=5, alpha=0.5, c='b')
#  # Plot the closing price of the symbol, and the closing price of the Dow Jones Industrial Average
#
#  ax[math.floor(stockID / 6), stockID % 6].set(title=symbolsOriginal[stockID]) # set title
#  ax[math.floor(stockID / 6), stockID % 6].tick_params(axis='both', labelsize=3, length = 0) # remove ticks
#
# for a in ax.flat:
...
# statisticallySignificantValues = list(filter(lambda x: x[1] <= 0.05, stockRankings))
#
...
# res['Weight'] = np.around(modelCoefficients, 3) # Round it to 3 places after the decimal
...
# title = title + ', R^2 = ' + str(np.around(model.score(dfObjTopNSymbols, Y), 3))
#
